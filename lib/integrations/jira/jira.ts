...
export class JiraAPI {
  ...
  async getProjectById(token: string, cloudId: string, projectId: string) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to get project by id ${response.status}`);
    }
  }
  ...
  async getAccessibleResources(token: string) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to get fetch resources : ${response.status}`);
    }
  ...
  async getCurrentUser(token: string, cloudId: string) {
    const response = await fetch(
      `${this.baseUrl}/${cloudId}/rest/api/3/project/myself`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
  ...
  async createProject(
    ...
    const response = await fetch(
      `${this.baseUrl}/${cloudId}/rest/api/3/project/project`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
