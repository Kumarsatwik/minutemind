...
export function useMeetingDetails(){
    ...
    // UI state
    const [activeTab,setActiveTab] = useState<'summary' | 'transcript'>('summary') // Currently active tab
    const [localActionItems,setLocalActionItems] = useState<any>([]) // Local copy of action items for optimistic updates
            ...
            try {
                ...
                if(meeting.transcript && !meeting.ragProcessed && userId == meeting.userId){
                    ...
                    if(typeof meeting.transcript === 'string'){
                        ...
                        transcriptText = meeting.transcript.map((item:any)=>`${item.speaker}: ${item.words.map((w:any)=>w.word).join(' ')}`).join('\n')
                    }
    ...
    const deleteActionItem = async(id:number)=>{
        ...
        setLocalActionItems((prev: any[]) => prev.filter((item:any)=>item.id !==id))
    }
    ...
    const meetingInfoData = meetingData ? {
        ...
        time:`${new Date(meetingData.startTime).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})} - ${new Date(meetingData.endTime).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}`,
